...
if (length(x)>5){
cat(" DALY ~ ")
for (i in 6:length(x)) cat(round(100*mean(x[[i]]$DALY)/mean(x$DALY),0), "%", ifelse(!nchar(x[[i]]$name)==0, paste(x[[i]]$name, " (outcome ", i-5, ")", sep=""), paste("outcome",i-5)), "\n\t")
cat("\n\n")
for (i in 6:length(x)) print(x[i])
}
}
